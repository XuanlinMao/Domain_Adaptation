...
def eval_all(y_true, y_pred, score, Klist=[50,100,200,300]) -> Dict:
    ...
    g_mean = eval_geometric_mean(y_true, y_pred)
...
# def eval_all_unsup(y_true, score, threshold=0.5, Klist=[50,100,200,300]) -> Dict:
...
@torch.no_grad()
def eval_acc(y_true, y_pred) -> float:
...
@torch.no_grad()
def eval_f1(y_true, y_pred) -> float:
    y_true = y_true.detach().cpu().numpy().flatten()
    y_pred = y_pred.detach().cpu().numpy().flatten()
...
@torch.no_grad()
def eval_precision(y_true, y_pred) -> float:
    y_true = y_true.detach().cpu().numpy().flatten()
    y_pred = y_pred.detach().cpu().numpy().flatten()
...
@torch.no_grad()
def eval_recall(y_true, y_pred) -> float:
    y_true = y_true.detach().cpu().numpy().flatten()
    y_pred = y_pred.detach().cpu().numpy().flatten()
...
@torch.no_grad()
def eval_geometric_mean(y_true, y_pred) -> float:
    y_true = y_true.detach().cpu().numpy().flatten()
    y_pred = y_pred.detach().cpu().numpy().flatten()
